...
describe "parser.rb" do
  ...
  context "with a more complex file" do
    # generated by head -n20 fixtures/webserver.log
    # then modified by hand to include non-unique IP's
    let(:fixture_path) { "spec/fixtures/complex.log" }

    it "returns the correct summary of uniques" do
      stdout, stderr, status = Open3.capture3(command)
      expect(status.success?).to eq(true)
      matcher = Regexp.new(<<~END_OF_REGEX
        /help_page/1 4 uniques
        /index 3 uniques
        /about 2 uniques
        /home 2 uniques
        /about/2 2 uniques
      ...
      expect(stdout).to match(matcher)
    end
  end
end